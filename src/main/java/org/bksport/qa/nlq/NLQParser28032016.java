 ...
 * 
 */
public class NLQParser28032016 {
	...
	public static Query parse2(String nlQuestion) {
		...
		if (newQuery != null) {
			...
			for (int i = 0; i < sp.size(); i++) 
			{
				...
				if (depEntryBefore.getDep().getTagName().startsWith("NNP") && !(doOr1)) 
				{
					if (depEntryBefore.getDep().getSlotEntry(0).getResource().getClass() == Var.class && !labeledOrSet.contains(depEntryBefore))
					{
						...
						Var label = new Var();
						newWhereOrClause.addTriple(new Triple(depEntryBefore.getDep().getSlotEntry(0).getResource(),new IRI("rdfs", "label"), label));
				...
				} else if (depEntryBefore.getDep().getTagName().startsWith("NN") && !(doOr2)) 
				{
					if (depEntryBefore.getDep().getSlotEntry(0).getType() != null) 
					{
						...
						newWhereOrClause.addTriple(new Triple(depEntryBefore.getDep().getSlotEntry(0).getResource(), new IRI("rdf", "type"), depEntryBefore.getDep().getSlotEntry(0).getType()));
					}
				}
				...
				if (depEntryAfter.getDep().getTagName().startsWith("NNP") && !(doOr3)) 
				{
					if (depEntryAfter.getDep().getSlotEntry(0).getResource().getClass() == Var.class && !labeledOrSet.contains(depEntryAfter)) 
					{
						...
						Var label = new Var();
						newWhereOrClause.addTriple(new Triple(depEntryAfter.getDep().getSlotEntry(0).getResource(), new IRI("rdfs", "label"), label));
				...
				} else if (depEntryAfter.getDep().getTagName().startsWith("NN") && !(doOr4)) 
				{
					if (depEntryAfter.getDep().getSlotEntry(0).getType() != null) 
					{
						...
						newWhereOrClause.addTriple(new Triple(depEntryAfter.getDep().getSlotEntry(0).getResource(), new IRI("rdf", "type"), depEntryAfter.getDep().getSlotEntry(0).getType()));
					}
				}
						...
						|| depEntryBefore.getGov().getValue().equals("'re")) 
				{
					...
					System.out.println("=== Truong hop cuoi cung ===");
					newWhereOrClause.addTriplePattern(
							new Triple(depEntryBefore.getDep().getSlotEntry(0).getResource(),
							depEntryBefore.getGov().getSlotEntry(0).getResource(), 
							depEntryAfter.getDep().getSlotEntry(0).getResource()));
							...
							|| depEntryBefore.getGov().getValue().equals("mention")) 
					{
						...
						if (j < depList2.size()) // --> ton tai prep_about()
						{
							...
							{
								newWhereOrClause.addTriple(new Triple(depEntryAfter.getDep().getSlotEntry(0).getResource(),
										IRI.RDF_TYPE_IRI, 
			...
			for (int i = 0; i < typedDepList.size(); i++) 
			{
				...
				if (DepEntry.isSupportType(typedDep.reln().getShortName())) 
				{
					...
					if (depEntry.getType().equals("prep")) 
					{
						if (depEntry.getSpecific() != null) 
						{
							if (whPhraseTree == null) {
								...
								{/*
									// riêng cho hỏi "news is about Chelsea"
									if (depEntry.getDep().getTagName().startsWith("NNP")) 
									{
										if (depEntry.getDep().getSlotEntry(0).getResource().getClass() == Var.class) 
										{
											...
											newWhereOrClause.addFilter(
																	new Constraint(new EqualExpression(new StrFunction(label),
																	new RDFLiteral(depEntry.getDep().getValue()))));
										}
									} else if (depEntry.getDep().getTagName().startsWith("NN") && depEntry.getDep().getSlotEntry(0).getType() != null
											&& depEntry.getDep().getSlotEntry(0).getType().equals(IRI.OWL_CLASS_IRI)) 
									{
										newWhereOrClause.addTriple(new Triple(depEntry.getDep().getSlotEntry(0).getResource(),
												new IRI("rdf", "type"),
												depEntry.getDep().getSlotEntry(0).getType()));
									}

									// ta phải xét ở đây để chọn được subject đúng, có 3 khả năng:
									// (1) Which news is about Lionel Messi? (có từ để hỏi)
									// (2) Lastest news about Lionel Messi? (không có từ để hỏi)
									// (3) News about Lionel Messi? 
									// cả 3 trường hợp subject đều là cái "news".
					...
					// lấy ví dụ cho nhánh dưới??? pobj chua thay xuat hien
					// trong cau hoi test nao cua minh
					if (depEntry.getType().equals("pobj")) 
					{
						if (depEntry.getGov().getValue().equals("about")) 
						{
							Var nVar = new Var();
							((SelectQuery) newQuery).addVar(nVar);
							if (depEntry.getDep().getTagName().startsWith("NNP")) 
							{
								if (depEntry.getDep().getSlotEntry(0).getResource().getClass() == Var.class) 
								{
									...
									newWhereOrClause.addFilter(new Constraint(
													new EqualExpression(new StrFunction(label),
													new RDFLiteral(depEntry.getDep().getValue()))));
								}
					...
					for (int i = 0; i < lexList.size(); i++) {
						if (lexList.get(i).getTagName().startsWith("W")) {
							if (lexList.get(i).getTagName().equals("WDT")) {
								...
								((SelectQuery) newQuery).addVar((Var) lexList
										.get(i + 1).getSlotEntry(0)
										.getResource());
								...
								if (nlQuestion.toLowerCase().contains(" news ")) 
								{
												...
												newWhereOrClause
														.addTriple(new Triple(
																dep.getDep()
																		...
																		.getResource(),
																new IRI(
																		"http://bk.sport.owl#isPublishedOn"),
																t));
												OrderCondiction ord = new OrderCondiction(
														t);
												ord.setDesc(true);
									...
									if (lexList.get(i + 1).getSlotEntry(0)
											.getType() != null) {
										newWhereOrClause.addTriple(new Triple(lexList.get(i + 1)
														.getSlotEntry(0)
														.getResource(),
														new IRI("rdf", "type"), 
														lexList
									...
									// newWhereOrClause.addTriple(new Triple((Var) lexList.get(i + 1).getSlotEntry(0).getResource(), new IRI(newQuery, "bksport","hasURL"), url));

									// thêm ngày 11-3-2015 để xử lý cho
									// "Which news is about Lionel Messi?"
									// for (int j = 0; j < typedDepList.size();
									// j++) {...}
								}
								...
								((SelectQuery) newQuery).addVar((Var) lexList
										.get(i).getSlotEntry(0).getResource());

								// Bo quan he rdfs:label voi who
								/*
								 * if (expectedResult ==
								 * PERSON_ORGANIZATION_RESULT) { Var label = new
								 * Var(); ((SelectQuery)
								 * newQuery).addVar(label);
								 * newQuery.getWhere().addTriplePattern( new
								 * Triple( (Var)
								 * lexList.get(i).getSlotEntry(0).getResource(),
								 * new IRI(newQuery, "rdfs", "label"), label));
								 * }
								 */
							...
							} else if (lexList.get(i).getTagName()
									.equals("WP$")) {// whose
								...
								} else if (lexList.get(i).getValue()
										.toLowerCase().equals("how")) {
									// TODO: implement SPARQL 1.1
									if (lexList.get(i + 1).getValue()
											.toLowerCase().equals("many")) {
										((SelectQuery) newQuery)
												.addVar(new Var(
														"COUNT("
																+ lexList
		...
		// ///// newQuery == null ///////
			...
			// Them bien (news) vao select (voi cau: news about Messi? Thing
			// about Messi?)
			for (int i = 0; i < lexList.size(); i++) {
				if (lexList.get(i).getTagName().startsWith("NN")) // vi du:
																	// news[NN]
																	// about
																	// Messi?
				{
